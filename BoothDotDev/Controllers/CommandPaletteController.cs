...
public sealed class CommandPaletteController : ControllerBase
{
    ...
    public IActionResult GetIndex()
    {
        ...
        entries.AddRange(_blogPostService.GetAllBlogPosts(visibility: visibility)
            .Select(post => new CommandPaletteEntry(post.Title, $"/blog/{post.Slug}", "Blog", PlainTextBody(post.CurrentDraft!.Body))));

        entries.AddRange(_noteService.GetAllNotes(visibility)
            .Select(note => new CommandPaletteEntry(note.Title, $"/note/{(ShortGuid)note.Id}", "Notes", PlainTextBody(note.CurrentDraft!.Content))));

        entries.AddRange(_devChallengeService.GetAllChallenges(visibility)
            .Select(challenge => new CommandPaletteEntry(challenge.Title, $"/challenge/{challenge.Id}", "Challenges", ChallengeBody(challenge.CurrentDraft!))));

        entries.AddRange(_tutorialService.GetAllArticles(visibility)
            .Select(article => new CommandPaletteEntry(article.Title, $"/learn/{_tutorialService.GetFullSlug(article)}", "Learn", PlainTextBody(article.CurrentDraft!.Body))));

        foreach (var project in _projectService.GetAllProjects())
        {
            entries.Add(new CommandPaletteEntry(project.Name, $"/project/{project.Slug}", "Projects", PlainTextBody(project.Details)));
            entries.AddRange(_projectService.GetDevlogs(project, visibility)
                .Select(devlog => new CommandPaletteEntry(devlog.Title, $"/project/{project.Slug}/devlog/{devlog.Slug}", "Projects", PlainTextBody(devlog.CurrentDraft!.Body))));
        }
